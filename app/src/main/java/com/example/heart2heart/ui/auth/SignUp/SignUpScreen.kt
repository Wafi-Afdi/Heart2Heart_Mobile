...
import com.example.heart2heart.utils.Phone_android
...
fun SignUpScreen(modifier: Modifier = Modifier, onLoginClicked: () -> Unit = {}, onRegisterSuccess: () -> Unit = {}) {
    ...
    Column(
        ...
        ClickableText(
            text = annotatedText,
            onClick = { offset ->
                // Find the annotation that spans the click position (offset)
                annotatedText.getStringAnnotations(
                    tag = LOGIN_TAG,
                    start = offset,
                    end = offset
                ).firstOrNull()?.let { annotation ->
                    // Check if the annotation tag matches our clickable part
                    if (annotation.tag == LOGIN_TAG) {
                        onLoginClicked() // Execute the provided callback function
